pacman::p_load(tidyverse,janitor,jsonlite,RMySQL,gmailr,bigrquery,googlesheets4,googledrive)
...
tryCatch({
    ...
    if(length(email_body) > 0){
        ...
        gm_auth(email = my_secrets$patches, path = "/home/cujo253/mines_of_moria/Essential_Referential_CSVS/gmail_ids.json", use_oob = T)
    ...
    {
        tryCatch({file.remove("/home/cujo253/mines_of_moria/Essential_Referential_CSVS/newspaper_safety.csv")},error=function(e){print("No CSV to Remove :)")})
    ...
    gm_auth_configure(path = "/home/cujo253/mines_of_moria/Essential_Referential_CSVS/gmail_ids.json")
